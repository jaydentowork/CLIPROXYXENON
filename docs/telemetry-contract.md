...
| `tokens.input_tokens`, `output_tokens`, `cached_tokens`, `reasoning_tokens` or flat counterparts | Available category counts, stored separately. Nested or flat `cache_read_tokens` is accepted; for native Claude, an explicit read count (including zero) overrides legacy `cached_tokens`, which can contain cache writes. A supplied read count is authoritative unless `cache_read_tokens_present` is explicitly false; an omitted presence flag does not invalidate the count. |
...
Unused raw fields—including source/email, endpoint, authentication metadata, and response headers—are discarded before persistence. The app does not log raw telemetry. Missing token values stay null in feed records unless Claude's full input can be derived from its source total and output. Provider totals sum the reported values; cached/reasoning categories are never added again to the source total. Dashboard `inputTokens` is inclusive of cached input. For native Claude, each stored request's `total_tokens - output_tokens` supplies the full prompt, including cache reads and writes, when it covers the known input and cache counts. Incomplete or older totals fall back to the available uncached input plus cache count. This normalization runs before aggregation and for feed rows, so existing history is corrected without rewriting raw input or changing pricing. Claude output already includes thinking. Historical rows that retained only the ambiguous legacy cache count cannot distinguish cache reads from writes. Two attempts with the same request ID remain separate records. xAI is excluded from storage, totals, and feed. Antigravity Claude-model requests remain included.
